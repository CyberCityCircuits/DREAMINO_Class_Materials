 ...
 * 
 * This version has been updated to fit the DREAMINO 5110 Gamer kit by David Ray
 * Arpil 11, 2019
 */
...
void sceneTitle() {
  ...
  while (gamestate == GAME_TITLE) {
   ...
   if (TIMER_CURRENT - TIMER_PREV > DELAY) { 
        ...
        inputManager(SW_02, SW_06, SW_03);
    ...
    TIMER_CURRENT = millis();
      ...
      if (TIMER_CURRENT - TIMER_PREV > DELAY) { 
        ...
        TIMER_PREV = TIMER_CURRENT;
          ...
          inputManager(SW_02, SW_01, SW_03);
  ...
  while (gamestate == GAME_PLAY) {
    ...
    TIMER_CURRENT = millis();
      ...
      if(TIMER_CURRENT - TIMER_PREV > DELAY) { // game loop
        ...
        inputManager(SW_02, SW_01, SW_03); // 5 = 0
...
void setup()   {
  ...
  digitalWrite(light_1, HIGH); delay(250);
  digitalWrite(light_2, HIGH); delay(250);
  digitalWrite(light_3, HIGH); delay(250);
  digitalWrite(light_1, LOW); delay(250);
  digitalWrite(light_2, LOW); delay(250);
  digitalWrite(light_3, LOW); delay(250);
  ...
  display.setContrast(SCR_CONTRAST);
